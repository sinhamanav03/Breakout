...
function LevelMaker.createMap(level)
    ...
    for y = 1, numRows do
        ...
        -- flag to check whether to spawn LockedBrick or not
        if not gLockedBrickExist and #bricks > (numCols * numRows )/4  then
            glockedBrickSpawn = math.random(1,9) % 4 == 0 and true or false
        end
        ...
        for x = 1, numCols do
            ...
            b = Brick(
                ...
                * 32                    -- multiply by 32, the brick width
                + 8                    -- the screen should have 16 pixels of padding; we can fit 13 cols + 16 pixels total
                + (13 - numCols) * 16,  -- left-side padding for when there are fewer than 13 columns
                
                -- y-coordinate
                y * 16 + 8                  -- just use y * 16, since we need top padding anyway
            )
            if glockedBrickSpawn then
                b.color = 6
                b.tier = 3
                gLockedBrickExist = true
                glockedBrickSpawn = false
            else
